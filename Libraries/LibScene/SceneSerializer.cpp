...
SceneSerializer::SceneSerializer(Core::Shared<Scene> const& scene)
    : m_Scene(scene)
{
}
...
static YAML::Node FindEntity(YAML::Node scene, Core::UUID const& entityID)
{
    for (auto entity : scene) {
        TR_CORE_TRACE(TR_LOG_CORE, entity);
        Core::UUID id = entity["Entity"].as<Core::UUID>();
        if (id == entityID)
            return entity;
    }
...
Core::Result<void, SceneSerializationError> SceneSerializer::DeserializeEditor(std::filesystem::path const& scenePath)
{
    YAML::Node data;
    try {
        ...
        TR_CORE_ERROR(TR_LOG_ASSET, ex.what());
        return { SceneSerializationError::InvalidFormat };
    } catch (YAML::BadFile const& ex) {
        TR_CORE_ERROR(TR_LOG_ASSET, ex.what());
        return { SceneSerializationError::NotFound };
    }
