...
char const* SceneSerializer::SceneFilter = "Terran Scene\0*.terran\0";
static char const* SerializerVersion = "yml1.0";

SceneSerializer::SceneSerializer(Terran::Core::Shared<Scene> const& scene)
    : m_Scene(scene)
{
}
        ...
        if (field.IsArray) {
            ...
            for (int32_t i = 0; i < length; i++) {
                switch (field.Type) {
                    ...
                    WRITE_SCRIPT_FIELD_ARRAY_ELEMENT(Vector3, glm::vec3);
                    WRITE_SCRIPT_FIELD_ARRAY_ELEMENT(Color, glm::vec4);
                    WRITE_SCRIPT_FIELD_ARRAY_ELEMENT(Entity, Terran::Core::UUID);
                default:
                    TR_ASSERT(false, "Unsupported field type");
                    break;
                }
            }
            out << YAML::EndSeq;
            ...
            switch (field.Type) {
                ...
                WRITE_SCRIPT_FIELD(Entity, Terran::Core::UUID);
            default:
                TR_ASSERT(false, "Unsupported field type");
                break;
            }
...
void SceneSerializer::SerializeEditor(std::filesystem::path const& scenePath)
{
    YAML::Emitter out;
    ...
    auto const tagComponentView = m_Scene->GetEntitiesWith<TagComponent>();
...
static void DeserializeScriptFields(Terran::Core::Shared<ScriptInstance> scriptInstance, ScriptComponent& scriptComponent, const YAML::Node& scriptFieldsNode)
{
    for (auto const& fieldID : scriptComponent.FieldHandles) {
        ...
        if (scriptField.IsArray) {
            ...
            for (int32_t i = 0; i < length && i < scriptFieldNode.size(); i++) {
                switch (scriptField.Type) {
                    ...
                    READ_SCRIPT_FIELD_ARRAY_ELEMENT(Entity, Terran::Core::UUID);
                default:
                    TR_ASSERT(false, "Invalid script type");
                }
            }
        } else {
            switch (scriptField.Type) {
                ...
                READ_SCRIPT_FIELD(Entity, Terran::Core::UUID);
            default:
                TR_ASSERT(false, "Invalid script type");
            }
        }
...
static YAML::Node FindEntity(YAML::Node scene, Terran::Core::UUID const& entityID)
{
...
static Entity DeserializeEntity(YAML::Node data, YAML::Node scene, Terran::Core::Shared<Scene> deserializedScene)
{
    try {
        Terran::Core::UUID id = data["Entity"].as<Terran::Core::UUID>();
        if (!id) {
        ...
        if (spriteRendererComponent) {
            ...
            src.TextureHandle = spriteRendererComponent["Texture"].as<Terran::Core::UUID>(Terran::Core::UUID::Invalid());
        }
        ...
        if (relationshipComponent) {
            ...
            for (auto childID : relationshipComponent["Children"]) {
                Terran::Core::UUID deserializedChildID = childID.as<Terran::Core::UUID>();
        ...
        if (scriptComponent) {
            ...
            Terran::Core::Shared<ScriptInstance> scriptInstance = ScriptEngine::CreateScriptInstance(deserializedEntity);
        ...
        if (rigidbodyComponent) {
            ...
            if (rigidbodyComponent["Material"])
                rbc.PhysicsMaterialHandle = rigidbodyComponent["Material"].as<Terran::Core::UUID>(Terran::Core::UUID::Invalid());
        }
...
Result SceneSerializer::DeserializeEditor(std::filesystem::path const& scenePath)
{
    YAML::Node data;
    try {
