...
struct BatchRenderer2DData final {
    ...
    Terran::Core::Shared<IndexBuffer> IndexBuffer;

    // ******** Quad ********
    uint32_t QuadIndexCount = 0;
    Terran::Core::Shared<VertexArray> QuadVertexArray;
    Terran::Core::Shared<VertexBuffer> QuadVertexBuffer;

    Terran::Core::Shared<Shader> QuadShader;

    QuadVertex* QuadVertexPtr = nullptr;
    uint32_t QuadVertexPtrIndex = 0;

    Terran::Core::Shared<Texture2D> QuadTextures[MaxTextureSlots];
    uint32_t QuadTextureIndex = 1;
    // **********************

    // ******** Circle ********
    uint32_t CircleIndexCount = 0;
    Terran::Core::Shared<VertexArray> CircleVertexArray;
    Terran::Core::Shared<VertexBuffer> CircleVertexBuffer;

    Terran::Core::Shared<Shader> CircleShader;
    ...
    Terran::Core::Shared<Shader> LineShader;

    Terran::Core::Shared<VertexArray> LineVertexArray;
    Terran::Core::Shared<VertexBuffer> LineVertexBuffer;
    ...
    // ******** Debug Line ********
    Terran::Core::Shared<Shader> DebugLineShader;

    Terran::Core::Shared<VertexArray> DebugLineVAO;
    Terran::Core::Shared<VertexBuffer> DebugLineVBO;
    ...
    Terran::Core::Shared<VertexBuffer> TextVBO;
    Terran::Core::Shared<VertexArray> TextVAO;

    Terran::Core::Shared<Shader> TextShader;

    TextVertex* TextVertexPtr = nullptr;
    uint32_t TextVertexPtrIndex = 0;

    Terran::Core::Shared<Texture2D> TextTextures[MaxTextureSlots];
    ...
    Terran::Core::Shared<UniformBuffer> CameraBuffer;
};
...
void BatchRenderer2D::Initialize(uint32_t batchSize)
{
    ...
    // ******** Quad ********
    {
        ...
        s_Data->QuadVertexArray = Terran::Core::CreateShared<VertexArray>();

        s_Data->QuadVertexBuffer = VertexBuffer::Create(s_Data->MaxVertices * sizeof(QuadVertex));

        s_Data->QuadVertexBuffer->SetLayout({ { VertexBufferElementType::Float, 3 },
        ...
        Terran::Core::Buffer whiteTextureBuffer = Terran::Core::Buffer(&whiteTextureData, sizeof(uint32_t));
    ...
    // ******** Circle ********
    {
        ...
        s_Data->CircleVertexArray = Terran::Core::CreateShared<VertexArray>();

        s_Data->CircleVertexBuffer = VertexBuffer::Create(s_Data->MaxVertices * sizeof(CircleVertex));

        s_Data->CircleVertexBuffer->SetLayout({ { VertexBufferElementType::Float, 3 },
    ...
    // ******** Line ********
    {
        ...
        s_Data->LineVertexArray = Terran::Core::CreateShared<VertexArray>();
        s_Data->LineVertexBuffer = VertexBuffer::Create(s_Data->MaxVertices * sizeof(LineVertex));

        s_Data->LineVertexBuffer->SetLayout({ { VertexBufferElementType::Float, 3 },
    ...
    // ******** Text ********
    {
        ...
        s_Data->TextVAO = Terran::Core::CreateShared<VertexArray>();
    ...
    // ******** Debug Line ********
    {
        ...
        s_Data->DebugLineVAO = Terran::Core::CreateShared<VertexArray>();
        s_Data->DebugLineVBO = VertexBuffer::Create(s_Data->MaxVertices * sizeof(DebugLineVertex));

        s_Data->DebugLineVBO->SetLayout({ { VertexBufferElementType::Float, 3 },
            { VertexBufferElementType::Float, 4 } });
...
void BatchRenderer2D::BeginFrame(Camera& camera, glm::mat4 const& transform, bool inverseView)
{
    TR_PROFILE_FUNCTION();
    Clear();
...
void BatchRenderer2D::AddQuad(glm::mat4& transform, glm::vec4 const& color, Terran::Core::Shared<Texture2D> texture, int entityID)
{
    TR_PROFILE_FUNCTION();
...
void BatchRenderer2D::AddQuad(glm::mat4& transform, glm::vec4 const& color, int entityID)
{
    TR_PROFILE_FUNCTION();
    AddQuad(transform, color, nullptr, entityID);
}

void BatchRenderer2D::AddQuad(glm::mat4& transform, glm::vec4 const& color, Terran::Core::Shared<Texture2D> texture, glm::vec2 textureCoordinates[4], int entityID)
{
    TR_PROFILE_FUNCTION();
    if (!QuadBatchHasRoom()) {
        // Begin New Batch
...
void BatchRenderer2D::AddCircle(glm::mat4& transform, glm::vec4 const& color, float thickness, int entityID)
{
    TR_PROFILE_FUNCTION();
...
void BatchRenderer2D::AddLine(glm::vec3 const& startPoint, glm::vec3 const& endPoint, glm::vec4 const& color, float thickness, int entityID)
{
    TR_PROFILE_FUNCTION();
    if (!LineBatchHasRoom()) {
        EndFrame();
...
void BatchRenderer2D::AddDebugLine(glm::vec3 const& startPoint, glm::vec3 const& endPoint, glm::vec4 const& color)
{
    TR_PROFILE_FUNCTION();
...
static glm::vec2 CalculateTextSize(float size, std::string const& text, Terran::Core::Shared<Font> font)
{
    TR_PROFILE_FUNCTION();
...
void BatchRenderer2D::AddString(glm::mat4& transform, std::string const& text, glm::vec4 const& color,
    Terran::Core::Shared<Font> font, float lineSpacing, float lineWidth, int entityID)
{
    TR_PROFILE_FUNCTION();
    ...
    msdfgen::FontMetrics const& fontMetrics = font->GetMetrics();
...
void BatchRenderer2D::AddDebugRect(glm::mat4 const& transform, glm::vec4 const& color)
{
    TR_PROFILE_FUNCTION();
...
{
    TR_PROFILE_FUNCTION();
    s_Data->Stats.VertexCount += s_Data->QuadVertexPtrIndex + s_Data->CircleVertexPtrIndex + s_Data->LineVertexPtrIndex + s_Data->DebugLineVertexPtrIndex + s_Data->TextVertexPtrIndex;

    s_Data->Stats.IndexCount += s_Data->QuadIndexCount + s_Data->CircleIndexCount + s_Data->LineIndexCount + s_Data->TextIndexCount;
...
bool BatchRenderer2D::TextBatchHasRoom()
{
    return false;
    !(s_Data->TextIndexCount >= s_Data->MaxIndices) && !(s_Data->TextTextureIndex >= s_Data->MaxTextureSlots);
}
