...
Terran::Core::Shared<Font> Font::DefaultFont;
...
{
    msdf_atlas::GlyphGeometry const* glyph = m_FontGeometry->getGlyph(c);
...
msdfgen::FontMetrics const& Font::GetMetrics() const
{
...
{
    std::map<std::pair<int, int>, double> const& kerningMap = m_FontGeometry->getKerning();
    msdf_atlas::GlyphGeometry const* glyph1 = m_FontGeometry->getGlyph(c1);
    msdf_atlas::GlyphGeometry const* glyph2 = m_FontGeometry->getGlyph(c2);
...
Terran::Core::Shared<Texture2D> Font::LoadFont(std::string const& fontPath)
{
    ...
    Terran::Core::Buffer data = Terran::Core::Buffer(bitmap.pixels, bitmap.width * bitmap.height * 3);
