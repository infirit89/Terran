...
ShaderCompiler::ShaderCompiler(std::filesystem::path const& shaderPath)
    : m_ShaderPath(shaderPath)
{
}

Terran::Core::Shared<Shader> ShaderCompiler::Compile()
{
    ...
    Terran::Core::Shared<Shader> shader = Terran::Core::CreateShared<Shader>(compiledShaders);
...
void ShaderCompiler::Recompile(Terran::Core::Shared<Shader> const& shader)
{
    Terran::Core::Shared<ShaderCompiler> compiler = Terran::Core::CreateShared<ShaderCompiler>(shader->m_Path);
    ...
    for (auto const& stage : sources | std::views::keys) {
...
std::vector<ShaderUnitInfo> ShaderCompiler::Compile_Internal(ShaderSourcesMap const& shaderSources, std::string const& shaderName)
{
    ...
    for (auto const& [stage, shaderSource] : shaderSources) {
...
#pragma stage : vertex
*/
ShaderCompiler::ShaderSourcesMap ShaderCompiler::Preprocess(std::string const& shaderSource)
{
    ShaderSourcesMap shaderSources;

    char const* shaderStageToken = "#shader_stage";
    size_t const typeTokenLength = strlen(shaderStageToken);
