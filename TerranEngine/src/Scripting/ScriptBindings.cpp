...
void ScriptBindings::Log_LogICall(uint8_t logLevel, Coral::String message)
{
    ...
    switch (logLevel) {
    case 1 << 0:
        TR_CLIENT_TRACE(messageStr);
        break;
    case 1 << 1:
        TR_CLIENT_WARN(messageStr);
        break;
    case 1 << 2:
        TR_CLIENT_ERROR(messageStr);
        break;
    default:;
    }
}
...
void* ScriptBindings::Input_GetConnectedControllersICall()
{
    ...
    Coral::ManagedArray connectedControllersArray = Coral::ManagedArray::New(*byteType, static_cast<int32_t>(connectedControllers.size()));
...
bool ScriptBindings::Entity_HasComponentICall(const Terran::Core::UUID& id, int32_t typeId)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
    if (!entity)
...
void ScriptBindings::Entity_AddComponentICall(Terran::Core::UUID const& id, int32_t typeId)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
    if (!entity)
...
void ScriptBindings::Entity_RemoveComponentICall(Terran::Core::UUID const& id, int32_t typeId)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
    if (!entity)
...
void* ScriptBindings::Entity_GetScriptableComponentICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
    if (!entity)
    ...
    Terran::Core::Shared<ScriptInstance> scriptInstance = ScriptEngine::GetScriptInstance(entity);
    return (void*)scriptInstance->GetHandle();
}

bool ScriptBindings::Entity_FindEntityWithNameICall(Coral::String entityName, Terran::Core::UUID& id)
{
...
void ScriptBindings::Entity_DestroyEntityICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
...
int ScriptBindings::Entity_GetChildrenCountICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
    if (!entity)
...
Terran::Core::UUID ScriptBindings::Entity_GetChildICall(Terran::Core::UUID const& id, int index)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
    if (!entity)
        return Terran::Core::UUID::Invalid();

    return entity.GetChild(index).GetID();
}
Terran::Core::UUID ScriptBindings::Entity_GetParentICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_ENTITY();
    if (!entity)
...
// ---- Tag Component ----
#pragma region Tag Component
Coral::String ScriptBindings::Tag_GetNameICall(const Terran::Core::UUID& id)
{
    GET_ENTITY();
    if (!entity)
...
void ScriptBindings::Tag_SetNameICall(Terran::Core::UUID const& id, Coral::String name)
{
    GET_ENTITY();
    if (!entity)
        return;
...
// ---- Transform Component ----
#pragma region Transform Component
glm::vec3 ScriptBindings::Transform_GetPositionICall(const Terran::Core::UUID& id)
{
    GET_COMPONENT_PROPERTY(TransformComponent, Position, glm::vec3(0.0f, 0.0f, 0.0f));
}

void ScriptBindings::Transform_SetPositionICall(Terran::Core::UUID const& id, glm::vec3 const& position)
{
    SET_TRANSFORM_COMPONENT_PROPERTY(Position, position);
}

glm::vec3 ScriptBindings::Transform_GetRotationICall(Terran::Core::UUID const& id)
{
    GET_COMPONENT_PROPERTY(TransformComponent, Rotation, glm::vec3(0.0f, 0.0f, 0.0f));
}

void ScriptBindings::Transform_SetRotationICall(Terran::Core::UUID const& id, glm::vec3 const& rotation)
{
    SET_TRANSFORM_COMPONENT_PROPERTY(Rotation, rotation);
}

glm::vec3 ScriptBindings::Transform_GetScaleICall(Terran::Core::UUID const& id)
{
    GET_COMPONENT_PROPERTY(TransformComponent, Scale, glm::vec3(1.0f, 1.0f, 1.0f));
}

void ScriptBindings::Transform_SetScaleICall(Terran::Core::UUID const& id, glm::vec3 const& scale)
{
    SET_TRANSFORM_COMPONENT_PROPERTY(Scale, scale);
}

bool ScriptBindings::Transform_IsDirtyICall(Terran::Core::UUID const& id)
{
    GET_COMPONENT_PROPERTY(TransformComponent, IsDirty, false);
}

glm::vec3 ScriptBindings::Transform_GetForwardICall(Terran::Core::UUID const& id)
{
    GET_COMPONENT_PROPERTY(TransformComponent, Forward, glm::vec3(0.0f, 0.0f, 1.0f));
}

glm::vec3 ScriptBindings::Transform_GetUpICall(Terran::Core::UUID const& id)
{
    GET_COMPONENT_PROPERTY(TransformComponent, Up, glm::vec3(0.0f, 1.0f, 0.0f));
}
glm::vec3 ScriptBindings::Transform_GetRightICall(Terran::Core::UUID const& id)
{
...
// ---- Sprite Renderer Component ----
#pragma region Sprite Renderer Component
glm::vec4 ScriptBindings::SpriteRenderer_GetColorICall(const Terran::Core::UUID& id)
{
    GET_COMPONENT_PROPERTY(SpriteRendererComponent, Color, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
}

void ScriptBindings::SpriteRenderer_SetColorICall(Terran::Core::UUID const& id, glm::vec4 const& color)
{
...
// ---- Camera Component ----
#pragma region Camera Component
bool ScriptBindings::Camera_IsPrimaryICall(const Terran::Core::UUID& id)
{
    GET_COMPONENT_PROPERTY(CameraComponent, Primary, false);
}
void ScriptBindings::Camera_SetPrimaryICall(Terran::Core::UUID const& id, bool primary)
{
    SET_COMPONENT_PROPERTY(CameraComponent, Primary, primary);
}

glm::vec4 ScriptBindings::Camera_GetBackgroundColorICall(Terran::Core::UUID const& id)
{
    GET_COMPONENT_PROPERTY(CameraComponent, BackgroundColor, glm::vec4(0.1f, 0.1f, 0.1f, 1.0f));
}
void ScriptBindings::Camera_SetBackgroundColorICall(Terran::Core::UUID const& id, glm::vec4 const& backgroundColor)
{
    SET_COMPONENT_PROPERTY(CameraComponent, BackgroundColor, backgroundColor);
}
glm::vec3 ScriptBindings::Camera_ScreenToWorldPointICall(Terran::Core::UUID const& id, glm::vec3 const& point)
{
    GET_ENTITY();
...
// ---- Circle Renderer Component ----
#pragma region Circle Renderer Component
glm::vec4 ScriptBindings::CircleRenderer_GetColorICall(const Terran::Core::UUID& id)
{
    GET_COMPONENT_PROPERTY(CircleRendererComponent, Color, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
}
void ScriptBindings::CircleRenderer_SetColorICall(Terran::Core::UUID const& id, glm::vec4 const& color)
{
    SET_COMPONENT_PROPERTY(CircleRendererComponent, Color, color);
}

float ScriptBindings::CircleRenderer_GetThicknessICall(Terran::Core::UUID const& id)
{
    GET_COMPONENT_PROPERTY(CircleRendererComponent, Thickness, 1.0f);
}

void ScriptBindings::CircleRenderer_SetThicknessICall(Terran::Core::UUID const& id, float thickness)
{
...
// ---- Text Renderer Component ----
#pragma region Text Renderer Component
glm::vec4 ScriptBindings::TextRenderer_GetColorICall(const Terran::Core::UUID& id)
{
    GET_COMPONENT_PROPERTY(TextRendererComponent, TextColor, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
}
void ScriptBindings::TextRenderer_SetColorICall(Terran::Core::UUID const& id, glm::vec4 const& color)
{
    SET_COMPONENT_PROPERTY(TextRendererComponent, TextColor, color);
}

Coral::String ScriptBindings::TextRenderer_GetTextICall(Terran::Core::UUID const& id)
{
    GET_ENTITY();
    if (!entity)
        return Coral::String::New("");
...
void ScriptBindings::TextRenderer_SetTextICall(Terran::Core::UUID const& id, Coral::String text)
{
    GET_ENTITY();
    if (!entity)
        return;
...
// ---- Scene ----
#pragma region Scene
Terran::Core::UUID ScriptBindings::Scene_GetMainCameraICall()
{
    Terran::Core::Shared<Scene> const& currentScene = SceneManager::GetCurrentScene();
...
{
    AssetInfo const& sceneAssetInfo = AssetManager::GetAssetInfoByPath(std::string(std::string(scenePath) + ".terran"));
    Terran::Core::Shared<Scene> loadedScene = AssetManager::GetAssetByAssetInfo<Scene>(sceneAssetInfo);
    if (!loadedScene)
        return false;
...
bool ScriptBindings::Physics2D_RayCastICall(const glm::vec2& origin, const glm::vec2& direction, float length, uint16_t layerMask, RayCastHitInfo2D_Internal& outHitInfo)
{
    ...
    if (!hitInfo.PhysicsBody) {
        outHitInfo.RigidbodyEntityId = Terran::Core::UUID::Invalid();
        return hasHit;
    }
...
void* ScriptBindings::Physics2D_RayCastAllICall(glm::vec2 const& origin, glm::vec2 const& direction, float length, uint16_t layerMask)
{
    TR_PROFILE_FUNCTION();
    Terran::Core::Shared<std::vector<RayCastHitInfo2D>> hitInfos = Physics2D::RayCastAll(origin, direction, length, layerMask);
...
#pragma region Rigidbody 2D
bool ScriptBindings::Rigidbody2D_IsFixedRotationICall(const Terran::Core::UUID& id)
{
    TR_PROFILE_FUNCTION();
    GET_RIGIDBODY_PROPERTY(FixedRotation, false);
}

void ScriptBindings::Rigidbody2D_SetFixedRotationICall(Terran::Core::UUID const& id, bool fixedRotation)
{
    TR_PROFILE_FUNCTION();
    SET_RIGIDBODY_COMPONENT_PROPERTY(FixedRotation, fixedRotation);
}

PhysicsBodySleepState ScriptBindings::Rigidbody2D_GetSleepStateICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_RIGIDBODY_PROPERTY(SleepState, PhysicsBodySleepState::Awake);
}
void ScriptBindings::Rigidbody2D_SetSleepStateICall(Terran::Core::UUID const& id, PhysicsBodySleepState sleepState)
{
    TR_PROFILE_FUNCTION();
    SET_RIGIDBODY_COMPONENT_PROPERTY(SleepState, sleepState);
}

float ScriptBindings::Rigidbody2D_GetGravityScaleICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_RIGIDBODY_PROPERTY(GravityScale, 0.0f);
}
void ScriptBindings::Rigidbody2D_SetGravityScaleICall(Terran::Core::UUID const& id, float gravityScale)
{
    TR_PROFILE_FUNCTION();
    SET_RIGIDBODY_COMPONENT_PROPERTY(GravityScale, gravityScale);
}

void ScriptBindings::Rigidbody2D_ApplyForceICall(Terran::Core::UUID const& id, glm::vec2 const& force, glm::vec2 const& position, ForceMode2D forceMode)
{
    TR_PROFILE_FUNCTION();
    SET_RIGIDBODY_PROPERTY(ApplyForce, force, position, forceMode);
}
void ScriptBindings::Rigidbody2D_ApplyForceAtCenterICall(Terran::Core::UUID const& id, glm::vec2 const& force, ForceMode2D forceMode)
{
    TR_PROFILE_FUNCTION();
    SET_RIGIDBODY_PROPERTY(ApplyForceAtCenter, force, forceMode);
}

glm::vec2 ScriptBindings::Rigidbody2D_GetLinearVelocityICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_RIGIDBODY_PROPERTY(LinearVelocity, glm::vec2(0.0f, 0.0f));
}
void ScriptBindings::Rigidbody2D_SetLinearVelocityICall(Terran::Core::UUID const& id, glm::vec2 const& linearVelocity)
{
    TR_PROFILE_FUNCTION();
    SET_RIGIDBODY_PROPERTY(SetLinearVelocity, linearVelocity);
}

float ScriptBindings::Rigidbody2D_GetAngularVelocityICall(Terran::Core::UUID const& id)
{
    TR_PROFILE_FUNCTION();
    GET_RIGIDBODY_PROPERTY(AngularVelocity, 0.0f);
}
void ScriptBindings::Rigidbody2D_SetAngularVelocityICall(Terran::Core::UUID const& id, float angularVelocity)
{
    SET_RIGIDBODY_PROPERTY(SetAngularVelocity, angularVelocity);
}

PhysicsBodyType ScriptBindings::Rigidbody2D_GetTypeICall(Terran::Core::UUID const& id)
{
    GET_RIGIDBODY_PROPERTY(BodyType, PhysicsBodyType::Dynamic);
}
void ScriptBindings::Rigidbody2D_SetTypeICall(Terran::Core::UUID const& id, PhysicsBodyType bodyType)
{
...
#define SET_COLLIDER2D_PROPERTY(Property, ColliderPropery, Value)                                    \
    ...
    Terran::Core::Shared<Collider2D> collider = nullptr;                                             \
                                                                                                     \
...
// ---- Collider 2D ----
#pragma region Collider 2D
void ScriptBindings::Collider2D_GetOffsetICall(const Terran::Core::UUID& id, ColliderType2D colliderType, glm::vec2& offset)
{
    TR_PROFILE_FUNCTION();
    constexpr glm::vec2 defaultValue = { 0.0f, 0.0f };
    GET_COLLIDER2D_PROPERTY(Offset, GetOffset, offset, defaultValue);
}
void ScriptBindings::Collider2D_SetOffsetICall(Terran::Core::UUID const& id, ColliderType2D colliderType, glm::vec2 const& offset)
{
    TR_PROFILE_FUNCTION();
    SET_COLLIDER2D_PROPERTY(Offset, SetOffset, offset);
}

void ScriptBindings::Collider2D_IsSensorICall(Terran::Core::UUID const& id, ColliderType2D colliderType, bool& sensor)
{
    TR_PROFILE_FUNCTION();
    GET_COLLIDER2D_PROPERTY(Sensor, IsSensor, sensor, false);
}

void ScriptBindings::Collider2D_SetSensorICall(Terran::Core::UUID const& id, ColliderType2D colliderType, bool isSensor)
{
    TR_PROFILE_FUNCTION();
...
#define SET_COLLIDER_PROPERY(ColliderComponent, Property, Value)                                                                                 \
    ...
    Terran::Core::Shared<ColliderComponent> collider = Terran::Core::DynamicCast<ColliderComponent>(physicsBody->GetCollider(bc.ColliderIndex)); \
    if (!collider)                                                                                                                               \
        return;                                                                                                                                  \
                                                                                                                                                 \
...
// ---- Box Collider 2D ----
#pragma region Box Collider 2D
void ScriptBindings::BoxCollider2D_GetSizeICall(const Terran::Core::UUID& id, glm::vec2& size)
{
    TR_PROFILE_FUNCTION();
    GET_COMPONENT_PROPERTY_NORET(BoxCollider2DComponent, Size, size);
    size = { 0.0f, 0.0f };
}
void ScriptBindings::BoxCollider2D_SetSizeICall(Terran::Core::UUID const& id, glm::vec2 const& size)
{
    TR_PROFILE_FUNCTION();
...
// ---- Circle Collider 2D ----
#pragma region Circle Collider 2D
float ScriptBindings::CircleCollider2D_GetRadiusICall(const Terran::Core::UUID& id)
{
    TR_PROFILE_FUNCTION();
    GET_COMPONENT_PROPERTY(CircleCollider2DComponent, Radius, 0.0f);
}
void ScriptBindings::CircleCollider2D_SetRadiusICall(Terran::Core::UUID const& id, float radius)
{
    TR_PROFILE_FUNCTION();
...
// ---- Capsule Collider 2D ----
#pragma region Capsule Collider 2D
void ScriptBindings::CapsuleCollider2D_GetSizeICall(const Terran::Core::UUID& id, glm::vec2& size)
{
    TR_PROFILE_FUNCTION();
    GET_COMPONENT_PROPERTY_NORET(CapsuleCollider2DComponent, Size, size);
    size = { 0.0f, 0.0f };
}
void ScriptBindings::CapsuleCollider2D_SetSizeICall(Terran::Core::UUID const& id, glm::vec2 const& size)
{
    TR_PROFILE_FUNCTION();
