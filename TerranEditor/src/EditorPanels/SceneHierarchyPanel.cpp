...
SceneHierarchyPanel::SceneHierarchyPanel(Terran::Core::Shared<Scene> const& scene)
{
    SetSceneContext(scene);
}

void SceneHierarchyPanel::SetSceneContext(Terran::Core::Shared<Scene> const& scene)
{
    m_Scene = scene;
}

void SceneHierarchyPanel::OnEvent(Terran::Core::Event& event)
{
...
void SceneHierarchyPanel::DrawScene()
{
    ...
    if (ImGui::BeginDragDropTargetCustom(currentWindow->Rect(), currentWindow->ID)) {
        if (ImGuiPayload const* payload = ImGui::AcceptDragDropPayload("ENTITY_UUID", ImGuiDragDropFlags_AcceptNoDrawDefaultRect)) {
...
bool SceneHierarchyPanel::OnKeyPressed(KeyPressedEvent& e)
{
    ...
    auto const& selected = SelectionManager::GetSelected(SelectionContext::Scene);
...
void SceneHierarchyPanel::DrawEntityNode(Entity entity)
{
    ...
    ImGuiTreeNodeFlags flags = (isSelected ? ImGuiTreeNodeFlags_Selected : 0) | ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_FramePadding | ImGuiTreeNodeFlags_AllowItemOverlap;
