...
static Terran::Core::Shared<Scene> s_CopyScene;
static Entity s_CopyEntity;

template<typename T>
static void DrawComponent(char const* label, Entity entity, UIFunc<T> func, bool removable = true)
{
    TR_PROFILE_FUNCTION();
...
{
    s_CopyScene = Terran::Core::CreateShared<Scene>();
...
static void DrawAddComponentButton(Entity entity, char const* label)
{
...
void PropertiesPanel::DrawComponents()
{
    ...
    std::vector<Terran::Core::UUID> const& selected = SelectionManager::GetSelected(SelectionContext::Scene);
    if (selected.empty())
        return;
    ...
    DrawComponent<SpriteRendererComponent>("Sprite Renderer", entity, [](SpriteRendererComponent& component) {
        UI::PropertyGroup("sprite_renderer", [&component]() {
            ...
            Terran::Core::Shared<Texture2D> texture = AssetManager::GetAssetByHandle<Texture2D>(component.TextureHandle);
            if (texture) {
                ImGui::TableNextRow();
    ...
    DrawComponent<ScriptComponent>("Script", entity, [&](ScriptComponent& component) {
        ...
        if (!component.FieldHandles.empty()) {
            ...
            for (auto const& fieldHandle : component.FieldHandles) {
    ...
    DrawComponent<Rigidbody2DComponent>("Rigidbody 2D", entity, [&](Rigidbody2DComponent& rbComponent) {
        ...
        ImGui::TableNextRow();
        if (UI::PropertyDropdown("Body Type", bodyTypeNames, rbComponent.BodyType) && isScenePlaying)
...

        {
            ...
            UI::ScopedStyleColor stlyeColor({ { ImGuiCol_HeaderHovered, { 1.0f, 0.0f, 0.0f, 0.0f } },
    ...
    DrawComponent<BoxCollider2DComponent>("Box Collider 2D", entity, [&](BoxCollider2DComponent& bcComponent) {
        ...
        Terran::Core::Shared<PhysicsBody2D> physicsBody = Physics2D::GetPhysicsBody(entity);
        Terran::Core::Shared<BoxCollider2D> boxCollider;

        if (isRuntime)
            boxCollider = Terran::Core::DynamicCast<BoxCollider2D>(physicsBody->GetColliders()[bcComponent.ColliderIndex]);
    ...
    DrawComponent<CircleCollider2DComponent>("Circle Collider 2D", entity, [&](CircleCollider2DComponent& ccComponent) {
        bool isRuntime = EditorLayer::GetInstace()->GetSceneState() == SceneState::Play;
        Terran::Core::Shared<PhysicsBody2D> physicsBody = Physics2D::GetPhysicsBody(entity);
        Terran::Core::Shared<CircleCollider2D> circleCollider;

        if (isRuntime)
            circleCollider = Terran::Core::DynamicCast<CircleCollider2D>(physicsBody->GetColliders()[ccComponent.ColliderIndex]);
    ...
    DrawComponent<CapsuleCollider2DComponent>("Capsule Collider 2D", entity, [&](CapsuleCollider2DComponent& ccComponent) {
        bool isRuntime = EditorLayer::GetInstace()->GetSceneState() == SceneState::Play;
        Terran::Core::Shared<PhysicsBody2D> physicsBody = Physics2D::GetPhysicsBody(entity);
        Terran::Core::Shared<CapsuleCollider2D> capsuleCollider;

        if (isRuntime && physicsBody)
            capsuleCollider = Terran::Core::DynamicCast<CapsuleCollider2D>(physicsBody->GetColliders()[ccComponent.ColliderIndex]);
