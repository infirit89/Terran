...
EditorLayer::EditorLayer(std::string const& projectPath)
    : Layer("Editor")
    , m_EditorCamera()
    , m_ProjectPath(projectPath)
{
    ...
    spdlog::sink_ptr editorConsoleSink = Terran::Core::CreateShared<EditorConsoleSink>();

    Terran::Core::Log::SetLoggerSink(TR_LOG_CLIENT, editorConsoleSink);
    Terran::Core::Log::SetLogFormat(TR_LOG_CLIENT, "%^[%T] %v%$");
}
...
void EditorLayer::OnAttach()
{
    ...
    Font::DefaultFont = Terran::Core::CreateShared<Font>("Resources/Fonts/Roboto/Roboto-Regular.ttf");

    AssetImporter::RegisterLoaders();

    // ***** Panel Setup *****
    m_PanelManager = Terran::Core::CreateShared<PanelManager>();
    m_PanelManager->AddPanel<LogPanel>(LOG_PANEL_NAME);
    m_PanelManager->AddPanel<PropertiesPanel>(PROPERTIES_PANEL_NAME);
    Terran::Core::Shared<ContentPanel> contentBrowserPanel = m_PanelManager->AddPanel<ContentPanel>(CONTENT_PANEL_NAME);
    contentBrowserPanel->SetOnItemClickCallback(TR_EVENT_BIND_FN(EditorLayer::OnContentBrowserItemClicked));
    Terran::Core::Shared<SceneViewPanel> sceneViewPanel = m_PanelManager->AddPanel<SceneViewPanel>(SCENE_VIEW_PANEL_NAME);
    ...
    Terran::Core::Shared<SettingsPanel> settingsPanel = m_PanelManager->AddPanel<SettingsPanel>(SETTINGS_PANEL_NAME);
    ...
    m_EditorSceneRenderer = Terran::Core::CreateShared<SceneRenderer>(editorFramebufferParams);

    sceneViewPanel->SetSceneRenderer(m_EditorSceneRenderer);
    ScriptEngine::LoadAppAssembly();

    Terran::Core::FileSystem::SetDirectoryToWatch(Project::GetAssetPath());
    Terran::Core::FileSystem::StartWatch();
...
{
    Terran::Core::FileSystem::StopWatch();
    EditorResources::Shutdown();
    Font::DefaultFont = nullptr;
}

void EditorLayer::Update(Terran::Core::Time& time)
{
    ...
    if (/*m_SceneViewPanel.IsVisible() &&*/ SceneManager::GetCurrentScene()) {
        ...
        case SceneState::Play: {
            AssetInfo const& sceneAssetInfo = AssetManager::GetAssetInfoByHandle(SceneManager::GetCurrentScene()->GetHandle());
...
void EditorLayer::OnEvent(Terran::Core::Event& event)
{
...
void EditorLayer::RenderDockspace()
{
    ...
    // render toolbar
    {
        ImGuiWindowFlags toolbarFlags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;
    ...
    if (ImGui::BeginMenuBar()) {
        auto menu = [](char const* name, std::function<void()> callback) {
...
{
    Terran::Core::Shared<LogPanel> logPanel = m_PanelManager->GetPanel<LogPanel>("LogPanel");
...
void EditorLayer::OnContentBrowserItemClicked(Terran::Core::Shared<ContentBrowserItem> const& item)
{
    if (item->GetType() != ItemType::File)
        return;

    AssetInfo const& assetInfo = AssetManager::GetAssetInfoByHandle(item->GetHandle());
...
{
    std::filesystem::path scenePath = Terran::Core::FileSystem::SaveFileDialog(SceneSerializer::SceneFilter);
    if (!scenePath.empty()) {
...
{
    std::filesystem::path scenePath = Terran::Core::FileSystem::OpenFileDialog(SceneSerializer::SceneFilter);
    AssetInfo assetInfo = AssetManager::GetAssetInfoByPath(scenePath);
    OpenSceneFromAssetPath(assetInfo, m_ViewportSize);
}

static bool IsValidAssetPath(std::filesystem::path const& path)
{
...
void EditorLayer::OpenSceneFromAssetPath(AssetInfo const& sceneAssetInfo, glm::vec2 const& viewportSize)
{
...
void EditorLayer::OpenProject(std::filesystem::path const& projectPath)
{
    // TODO: close project if theres an already active one
    Terran::Core::Shared<Project> project = Terran::Core::CreateShared<Project>(projectPath);
